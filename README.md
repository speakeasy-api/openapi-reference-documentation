...
Security scheme details are defined in the components section of a schema<sup><a href="https://github.com/OAI/OpenAPI-Specification/blob/main/versions/3.1.0.md#security-scheme-object">3.1</a></sup>. A security scheme object can be given any name, but its name must match the name used elsewhere in the document for the scheme to be used. For example:
...
First consider the fields that do not depend on type and can be used in any security scheme. They are all strings.
...
<!-- `identifier` | Human readable information. Another name for the scheme, separate to the name at the start of the object. | | | :heavy_check_mark:
`x-` | Extension fields | | | -->
...
- `type: http`
  - `scheme:` `basic` `bearer` `digest` — The name of the HTTP Authorization scheme to be used in the Authorization header. [More values](https://www.iana.org/assignments/http-authschemes/http-authschemes.xhtml) are theoretically allowed, but not supported in practice.
- `type: mutualTLS`
- `type: oauth2`
- `type: openIdConnect`
